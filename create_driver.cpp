    ...
    is_running_slowly_(false)
{
  ...
  song_sub_ = nh.subscribe("song", 10, &CreateDriver::songCallback, this);
...
void CreateDriver::songCallback(const std_msgs::BoolConstPtr& msg)
{
  if(msg->data) {
    const uint8_t& songNumber = 0;
    const uint8_t& songLength = 1;
    const uint8_t noteA[1] = {60}
    const uint8_t *note;
    note = noteA;
    ...
    bool songDefined = robot_->defineSong(songNumber, songLength, note, duration);
    bool songPlayed = robot_->playSong(songNumber);
  }
}
